...
namespace DoAn_LapTrinhWeb.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            //hiển thị những sản phẩm hot được mua nhiều nhất trong vòng 30 ngày gần nhất
            ...
            var aaa = db.Order_Detail.Where(s => s.create_at > getDate).ToList();
            List<Product> hotproductmobile = (from a in db.Products.Where(item => item.status == "1" && item.quantity != "0")
                                              join b in db.Order_Detail.Where(s => s.create_at > getDate)
                                              on a.product_id equals b.product_id
                                              select a).Take(4).OrderByDescending(item => item.buyturn).ToList();
            ViewBag.HotProductmobile = hotproductmobile;
